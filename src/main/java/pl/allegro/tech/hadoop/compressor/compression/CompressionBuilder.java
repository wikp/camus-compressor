    ...
    public class CompressorInputFileFormatBuilder {
        ...
        public Compression<AvroWrapper<GenericRecord>, NullWritable> forAvroFiles() {
            ...
            if (CompressionFormat.DEFLATE.equals(compressionFormat)) {
                return new DeflateCompression<>(sparkContext, fileSystem, avroKeyClass, avroValueClass, avroInputFormat,
                        avroOutputFormat);
            } else if (CompressionFormat.SNAPPY.equals(compressionFormat)) {
                return new SnappyCompression<>(sparkContext, fileSystem, avroKeyClass, avroValueClass, avroInputFormat,
                        avroOutputFormat);
            } else if (CompressionFormat.LZO.equals(compressionFormat)) {
                return new LzoCompression<>(sparkContext, fileSystem, avroKeyClass, avroValueClass, avroInputFormat,
                        avroOutputFormat);
            } else if (CompressionFormat.NONE.equals(compressionFormat)) {
                return new NoneCompression<>(sparkContext, fileSystem, avroKeyClass, avroValueClass, avroInputFormat,
                        avroOutputFormat);
            } else {
        ...
        public Compression<LongWritable, Text> forJsonFiles() {
            if (CompressionFormat.DEFLATE.equals(compressionFormat)) {
                return new DeflateCompression<>(sparkContext, fileSystem, jsonKeyClass, jsonValueClass, jsonInputFormat,
                        jsonOutputFormat);
            } else if (CompressionFormat.SNAPPY.equals(compressionFormat)) {
                return new SnappyCompression<>(sparkContext, fileSystem, jsonKeyClass, jsonValueClass, jsonInputFormat,
                        jsonOutputFormat);
            } else if (CompressionFormat.LZO.equals(compressionFormat)) {
                return new LzoCompression<>(sparkContext, fileSystem, jsonKeyClass, jsonValueClass, jsonInputFormat,
                        jsonOutputFormat);
            } else if (CompressionFormat.NONE.equals(compressionFormat)) {
                return new NoneCompression<>(sparkContext, fileSystem, jsonKeyClass, jsonValueClass, jsonInputFormat,
                        jsonOutputFormat);
            } else {
                throw new IllegalArgumentException("Invalid compression format provided");
            }
