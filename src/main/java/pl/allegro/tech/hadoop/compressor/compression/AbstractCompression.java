...
public abstract class AbstractCompression<K, V, I extends InputFormat<K, V>, O extends OutputFormat<K, V>>
        implements Compression<K, V> {
    ...
    AbstractCompression(JavaSparkContext sparkContext, Class<K> kClass, Class<V> vClass, Class<I> iClass,
                        Class<O> oClass) {
    ...
    public void compress(JavaPairRDD<K, V> content, String outputDir, JobConf jobConf)
            throws IOException {

        setupJobConf(jobConf);
        content.saveAsHadoopFile(outputDir, kClass, vClass, oClass, jobConf);
    }
